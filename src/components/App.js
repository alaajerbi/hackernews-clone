...
class App extends Component {
  ...
  componentDidMount() {
    ...
    api.fetchStoriesByTag(tag, page).then(data => {
      this.setState({
        stories: data.hits,
        loading: false,
  ...
  handleNextClick() {
    ...
    this.setState(
      ...
      () => {
        if (keyword === '') {
          api.fetchStoriesByTag(tag, this.state.page).then(data => {
            this.setState({
              stories: data.hits,
              loading: false,
              nbPages: data.nbPages
          ...
          api.fetchStoriesByKeyword(keyword, this.state.page).then(data => {
            this.setState({
              stories: data.hits,
              loading: false,
              keyword: keyword,
  ...
  handleSearchChange(keyword) {
    ...
    if (keyword === '') {
      ...
      api.fetchStoriesByTag(tag).then(data => {
        this.setState({
          stories: data.hits,
          loading: false,
          keyword: '',
      ...
      api.fetchStoriesByKeyword(keyword).then(data => {
        this.setState({
          stories: data.hits,
          loading: false,
          keyword: keyword,
  ...
  render() {
    ...
    return (
      <div className="container">
        ...
        <TagNav selectedTag={this.state.tag} onChange={this.handleTagChange} />
        <Stories
          loading={loading}
          stories={stories}
          keyword={keyword}
